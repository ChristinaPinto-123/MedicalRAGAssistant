...
class MedicalQueryRequest(BaseModel):
    query: str = Field(
        ..., 
        examples=["What is the first-line therapy for severe eosinophilic asthma?"]
    )
...
class SourceCitation(BaseModel):
    ...
    evidence_level_weight: float = Field(
        ..., 
        description="Weight from 0.4 to 1.0 based on evidence hierarchy"
    )
    snippet: str
...
class MedicalQueryResponse(BaseModel):
    ...
    confidence_score: float = Field(
        ..., 
        description="Calculated composite confidence (0.0 - 1.0)"
    )
